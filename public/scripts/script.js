...
*/
function handleEvents(element, event, funct, type = "add", params = []) {
  var newFunct = () => {
    if (params.length == 0) { 
      funct()
    } 
...
function displayLandingPageForms() {
  ...
  if (
    this == loginButtons[0] ||
    this == document.getElementsByClassName("login-opt")[0].children[0]
  ) {
    ...
    this == loginButtons[1].children[0] ||
    this == document.getElementsByClassName("signup-opt")[0].children[0]
  ) {
